...
import { StockOutRequest, Product, Warehouse } from '../types'
...
export default function StockOutForm({ products, warehouses, onSubmit, onCancel, isLoading = false }: StockOutFormProps) {
  ...
  const [errors, setErrors] = useState<Partial<Record<keyof StockOutRequest, string>>>({})
  ...
  const validateForm = (): boolean => {
    const newErrors: Partial<Record<keyof StockOutRequest, string>> = {}
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Error submitting stock out form:', error)
    }
  }

  const handleChange = (field: keyof StockOutRequest, value: string | number | undefined) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }))
  ...
  return (
    <div className="card">
      <div className="card-header">
        ...
        <p className="text-sm text-gray-600">Remove stock from warehouse</p>
      </div>
