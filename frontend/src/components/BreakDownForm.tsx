...
import { BreakDownRequest, Product, Warehouse } from '../types'

interface BreakDownFormProps {
  products: Product[]
  warehouses: Warehouse[]
  onSubmit: (data: BreakDownRequest) => Promise<void>
  onCancel: () => void
  isLoading?: boolean
}

export default function BreakDownForm({ products, warehouses, onSubmit, onCancel, isLoading = false }: BreakDownFormProps) {
  const [formData, setFormData] = useState<BreakDownRequest>({
    product_id: 0,
    warehouse_id: 0,
    qty_unit2: 0,
    note: ''
  ...
  const [errors, setErrors] = useState<Partial<Record<keyof BreakDownRequest, string>>>({})
  ...
  const validateForm = (): boolean => {
    const newErrors: Partial<Record<keyof BreakDownRequest, string>> = {}
    ...
    if (selectedProduct && !selectedProduct.unit2) {
      newErrors.product_id = 'Selected product must have a secondary unit for breakdown'
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Error submitting breakdown form:', error)
    }
  }

  const handleChange = (field: keyof BreakDownRequest, value: string | number) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }))
  ...
  return (
    <div className="card">
      <div className="card-header">
        <h3 className="text-lg font-medium text-gray-900">Break Down</h3>
        <p className="text-sm text-gray-600">Convert from larger units to smaller units</p>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {conversionInfo && (
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              <h4 className="text-sm font-medium text-blue-900 mb-2">Conversion Preview</h4>
              <div className="text-sm text-blue-800">
                <p>
                  Breaking down <strong>{conversionInfo.unit2Qty} {conversionInfo.unit2}</strong> will result in:
                </p>
                <p className="mt-1">
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t">
            ...
            <button
              ...
              disabled={isLoading}
            >
              {isLoading ? 'Processing...' : 'Break Down'}
            </button>
          </div>
        </form>
      </div>
